...
osMessageQueueId_t guiKeyQueue;
...
void SoundTask_Entry(void *argument)
{
    uint8_t soundType = 0;
    for(;;)
    {
        if (osMessageQueueGet(soundQueueHandle, &soundType, NULL, osWaitForever) == osOK)
        {
            switch(soundType)
            {
                case SOUND_START:
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  User_Keypad_Init();
  guiKeyQueue = osMessageQueueNew(5, sizeof(uint8_t), NULL);
  Buzzer_Init();
  const osMessageQueueAttr_t soundQueue_attributes = { .name = "soundQueue" };
  soundQueueHandle = osMessageQueueNew(10, sizeof(uint8_t), &soundQueue_attributes);
  ...
  soundTaskHandle = osThreadNew(SoundTask_Entry, NULL, &soundTask_attributes);
  /* USER CODE END 2 */
...
void StartDefaultTask(void *argument)
{
  ...
  uint8_t curr_2, curr_3, curr_13, curr_14;
  uint8_t prev_2 = 1, prev_3 = 1, prev_13 = 1, prev_14 = 1;
  uint8_t keyToSend = 0;
...
//    osDelay(500);
	...
	curr_2  = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_2);
	curr_3  = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_3);
	curr_13 = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_13);
	curr_14 = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_14);

	keyToSend = 0;

	if (curr_2 == 0 && prev_2 == 1)
	{
		keyToSend = 1;
		sprintf(buf, "KEY PRESSED: PG2 (UP)\r\n");
		HAL_UART_Transmit(&huart1, (uint8_t*)buf, strlen(buf), 100);
	}
	prev_2 = curr_2;

	if (curr_3 == 0 && prev_3 == 1)
	{
		keyToSend = 2;
		sprintf(buf, "KEY PRESSED: PG3 (DOWN)\r\n");
		HAL_UART_Transmit(&huart1, (uint8_t*)buf, strlen(buf), 100);
	}
	prev_3 = curr_3;

	if (curr_13 == 0 && prev_13 == 1)
	{
		keyToSend = 3;
		sprintf(buf, "KEY PRESSED: PG13 (LEFT)\r\n");
		HAL_UART_Transmit(&huart1, (uint8_t*)buf, strlen(buf), 100);
	}
	prev_13 = curr_13;

	if (curr_14 == 0 && prev_14 == 1)
	{
		keyToSend = 4;
		sprintf(buf, "KEY PRESSED: PG14 (RIGHT)\r\n");
		HAL_UART_Transmit(&huart1, (uint8_t*)buf, strlen(buf), 100);
	}
	prev_14 = curr_14;
	if (keyToSend != 0)
	{
	  ...
	  if (guiKeyQueue != NULL)
	  {
		  osMessageQueuePut(guiKeyQueue, &keyToSend, 0, 0);
	  }
	}
